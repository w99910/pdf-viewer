...
// node_modules/js-utils/src/Carbon.js
var Carbon = class {
  constructor(date) {
    this.date = typeof date === "string" || typeof date === "number" ? new Date(date) : date;
  }
  toHumanReadable() {
...
// node_modules/js-utils/src/to-human-readable-filesize.ts
function toHumanReadableFileSize(bytes, si = false, dp = 1) {
...
// node_modules/js-utils/src/Random.ts
var Random = class {
...
// node_modules/js-utils/assets/icons.js
var info = `<svg class="w-full h-full " xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="12" cy="12" r="10"></circle>
            <line x1="12" y1="16" x2="12" y2="12"></line>
            <line x1="12" y1="8" x2="12.01" y2="8"></line>
        </svg>`;
var error = `<svg class="w-full h-full" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>`;
var success = `<svg class="w-full h-full" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"></path><path d="m9 12 2 2 4-4"></path></svg>`;
var warning = `<svg class='w-full h-full' width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>`;
...
// node_modules/js-utils/src/Toaster.ts
var instance;
var Toaster = class {
  constructor(element) {
  ...
  setConfigOfType(type, config) {
    if (!this.config.hasOwnProperty(type)) {
      return false;
    }
    Object.keys(config).forEach((key) => {
      if (this.config[type].hasOwnProperty(key)) {
        this.config[type][key] = config[key];
      }
    });
    return this;
  }
  ...
  static toast(message, type, duration = 2e3) {
    if (!instance) {
...
// node_modules/js-utils/src/String.ts
var String = class {
...
// node_modules/js-utils/src/Tooltip.ts
var Tooltip = class {
  constructor(parent) {
    this.parent = parent;
    __publicField(this, "parentRect");
    __publicField(this, "windowRect");
    this.parentRect = parent.getBoundingClientRect();
  }
  setTooltip(tooltip, config = { position: "bottom", padding: 5 }) {
    tooltip.style.position = "fixed";
    let tooltipRect = tooltip.getBoundingClientRect();
    let rectWidth = tooltipRect.width;
    switch (config.position) {
      case "top":
        tooltip.style.top = `${this.parentRect.top - tooltipRect.height - config.padding}px`;
        let centerLeftTop = this.parentRect.left + this.parentRect.width / 2;
        tooltip.style.left = `${centerLeftTop - tooltipRect.width / 2}px`;
        break;
      case "bottom":
        tooltip.style.top = `${this.parentRect.top + this.parentRect.height + config.padding}px`;
        let centerLeft = this.parentRect.left + this.parentRect.width / 2;
        tooltip.style.left = `${centerLeft - tooltipRect.width / 2}px`;
        break;
      case "left":
        tooltip.style.top = `${this.parentRect.top + this.parentRect.height / 2 - tooltipRect.height / 2}px`;
        tooltip.style.left = `${this.parentRect.left - tooltipRect.width - config.padding}px`;
        break;
      case "right":
        tooltip.style.top = `${this.parentRect.top + this.parentRect.height / 2 - tooltipRect.height / 2}px`;
        tooltip.style.left = `${this.parentRect.left + this.parentRect.width + config.padding}px`;
        break;
    }
    this.adjustTopPosition(tooltip, config);
    this.adjustLeftPosition(tooltip, config);
  }
  adjustTopPosition(tooltip, config) {
      ...
      if (tooltipRect.height > this.parentRect.top) {
        ...
        switch (config.position) {
          case "bottom":
            ...
            break;
          case "left":
            tooltip.style.top = `${this.parentRect.top - tooltipRect.height / 2}px`;
            break;
        }
      }
    }
    if (tooltipRect.top < 0) {
      tooltip.style.top = config.position === "bottom" || config.position === "top" ? `${this.parentRect.bottom + config.padding}px` : `${config.padding}px`;
    }
  }
  adjustLeftPosition(tooltip, config) {
    const tooltipRect = tooltip.getBoundingClientRect();
    if (tooltipRect.left + tooltipRect.width > window.innerWidth) {
      tooltip.style.left = `${window.innerWidth - tooltipRect.width - config.padding}px`;
    }
    if (tooltipRect.left < 0) {
      tooltip.style.left = config.position === "bottom" || config.position === "top" ? `config.paddingpx` : `${this.parentRect.right + config.padding}px`;
    }
  }
};

// node_modules/js-utils/src/OnClickOutside.ts
